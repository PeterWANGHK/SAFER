...
**Not included in the source package (host separately):**
- Pretrained checkpoints (`rl/checkpoints/**/final.zip`) — large; host on Zenodo / OneDrive / HF Hub and link below.
- Raw naturalistic datasets (highD / exiD / rounD / inD / UTE) — distribute via the original sources only.
- Generated logs and figures (`rl/logs/`) — regenerated by the commands above.

External hosting (placeholder):
- Pretrained MetaDrive checkpoints: `https://<doi-or-zenodo-link>`
- Pretrained PINN checkpoints: same link, folder `pinn/`

## Computational Environment

> All training and evaluation were performed on a single workstation equipped with an **NVIDIA GeForce RTX 5080 GPU (16 GB VRAM)**, running **PyTorch 2.11 with CUDA 12.8** and **Stable-Baselines3** on **MetaDrive 0.4.3** / **HighwayEnv**; each MetaDrive policy was trained for **10⁶ environment steps with four parallel rollouts**, and inference latency was measured with policies on CPU to reflect deployment-time cost.

## Citation

```bibtex
@article{wang2025safead,
  title  = {SAFE-AD: Socially-Aware Field-Enhanced Reinforcement Learning for Autonomous Driving in Interactive Traffic},
  author = {Wang, Zian and Huang, Wenjie and Deng, Zejian and Shu, Yiming and Xu, Jiahui and Wang, Yong and Li, Shen and Cao, Dongpu and Sun, Chen},
